  ...
  <div class="VehicleWorkTyppe">
      ...
      <div class="oh w100b">
        <a-space direction="vertical" class="w100b">
          <a-table size="middle" :data-source="dataSource" :loading="tableLoading" :pagination="false">
            <a-table-column title="作业类型" data-index="typeName" align="center" :ellipsis="true" />
    ...
    <a-modal :title="operationTitle" :visible="operationVisible" :confirm-loading="operationLoading" @ok="operationOk" @cancel="operationVisible = false" :maskClosable="false" :width="600">
      <a-form-model ref="ruleForm" :model="typeForm" :rules="rules" :label-col="{ span: 6, offset: 0 }" :wrapper-col="{ span: 16, offset: 1 }">
        <a-form-model-item ref="vehicle_work_type" label="车辆作业类型" prop="vehicle_work_type" has-feedback>
          <a-input v-model="typeForm.vehicle_work_type" placeholder="请输入车辆作业类型" class="w100b" allowClear />
        </a-form-model-item>
      </a-form-model>
    </a-modal>
  </div>
...
export default {
  ...
  data() {
    return {
      ...
      total: 0,
      rules: {
        vehicle_work_type: [{ required: true, message: "请输入车辆作业类型", trigger: "blur" }]
      },
      edit_id: ""
    }
  },
  ...
  methods: {
    ...
    getTableList() {
      ...
      this.$api.getVehicleWorkTypeList(data).then(res => {
    ...
    operation(type, record = null) {
      ...
      if (type === "add") {
        this.operationTitle = "新增车辆作业类型"
        this.typeForm.vehicle_work_type = ""
      } else if (type === "edit") {
        this.operationTitle = "编辑车辆作业类型"
        this.typeForm.vehicle_work_type = record.typeName || ""
    ...
    del(record) {
      ...
      that.$api.delVehicleWorkType({ id: record.id }).then(res => {
        that.$confirm({
          title: "确认删除?",
          content: h => <div style="color:red;">确认删除该作业类型么？</div>,
          onOk() {
            that.operationVisible = false
            that.getTableList()
            that.$message.success(res.msg)
          },
          onCancel() {
    ...
    operationOk() {
      this.$refs.ruleForm.validate(valid => {
        if (valid) {
          let api =
            this.operationTitle === "新增车辆作业类型"
              ? this.$api.saveVehicleWorkType({ typeName: this.typeForm.vehicle_work_type })
              : this.$api.updateVehicleWorkType({ id: this.edit_id, typeName: this.typeForm.vehicle_work_type })
          api.then(res => {
            this.getTableList()
