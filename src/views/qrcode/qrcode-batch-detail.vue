    ...
    <a-space direction="vertical" size="large" class="w100b">
      <div class="oh">
        <a-space class="fl mb10" size="middle">
          <div>
            <span>二维码批次：</span>
            <div class="dib"><a-input v-model="filter.batch_no" placeholder="请输入二维码编码" disabled style="width:180px" allowClear /></div>
          </div>
          <div>
            <span>是否绑定：</span>
            <div class="dib"><a-select v-model="filter.is_bind" :options="bindOptions" placeholder="请选择是否绑定" style="width:120px" allowClear /></div>
          </div>
          <div>
            <span>绑定时间：</span>
            <div class="dib"><a-range-picker v-model="dateString" @change="onChangeBind" style="width:220px" /></div>
            <!-- <a-date-picker v-model="filter.bind_time" placeholder="请选择绑定时间" class="w100b" allowClear /> -->
          </div>
          <div>
            <span>绑定居民：</span>
            <div class="dib"><a-input v-model="filter.user_name" placeholder="请输入居民信息" style="width:150px" allowClear /></div>
          </div>
          <div>
            <span>创建时间：</span>
            <div class="dib"><a-range-picker v-model="dateString2" @change="onChange" style="width:220px" /></div>
      ...
      <div class="oh w100b">
        <a-space direction="vertical" class="w100b">
          <a-table size="middle" :data-source="dataSource" :loading="tableLoading" :pagination="false">
            ...
            <a-table-column title="绑定来源" data-index="bind_source" align="center" :ellipsis="true" />
...
export default {
  ...
  created() {
    this.filter.batch_no = this.$route.query.no
    this.getTableList(1)
  },
  ...
  methods: {
    ...
    getTableList(page) {
      ...
      let data = {
        ...
        batch_num: this.filter.batch_no,
